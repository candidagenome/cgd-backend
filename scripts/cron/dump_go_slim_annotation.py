...
def dump_go_slim_annotation(output_dir: Path | None = None) -> bool:
    ...
    gaf_file = output_dir / f"GOslim_gene_association.{PROJECT_ACRONYM.lower()}"
    today = datetime.now().strftime("%Y-%m-%d")
    today_tag = datetime.now().strftime("%Y%m%d")

    # Archive existing file
    if gaf_file.exists():
        archive_file = ARCHIVE_DIR / f"GOslim_gene_association.{PROJECT_ACRONYM.lower()}_{today_tag}"
        try:
    ...
    try:
        with SessionLocal() as session:
            ...
            logger.info(f"Wrote {annotation_count} GO Slim annotations to {gaf_file}")
