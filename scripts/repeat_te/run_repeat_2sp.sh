#!/usr/bin/env bash
# De novo repeat/TE pipeline for C. auris + C. parapsilosis — identical to the
# tropicalis pilot: RepeatModeler2 -LTRStruct -> TEsorter -> RepeatMasker(own lib)
# -> candidate TSV. Sequential (2-core box).
set -u
...
run_sp C_auris_B8441 CAURIS /data/genomes/C_auris_B8441_current_chromosomes.fasta
run_sp C_parapsilosis_CDC317 CPARA /data/genomes/C_parapsilosis_CDC317_current_chromosomes.fasta
echo "==== ALL DONE $(date) ===="
