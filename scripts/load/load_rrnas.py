#!/usr/bin/env python3
"""
Insert rRNA features for a species from a barrnap prediction (subtask-2 of the
"missing non-coding features" project).

BACKGROUND / why this is an INSERT (not a reclassify like the tRNA subtask):
  C. tropicalis (org 12) and C. dubliniensis CD36 (org 9) both had rRNA=0. Unlike
  the tRNAs (which existed mislabeled as ORFs), the ribosomal RNAs are genuinely
  ABSENT from CGD:
    - C. tropicalis: no rDNA features at all; the ASM633v3 reference GFF the gene
      models were lifted from has zero rRNA lines.
    - C. dubliniensis: curators loaded only ITS1/ITS2 (as ncRNA) on ChrR and left
      the flanking 18S/5.8S/25S/5S unannotated (the odd systematic name slots
      Cd36_34353/_34355/_34357 were reserved but never filled).
  So the rRNAs are predicted de novo with barrnap and inserted as new features.

WHAT THIS SCRIPT DOES, per planned rRNA:
  1. INSERT parent FEATURE (feature_type='rRNA', gene_name, headline, dbxref CAL...)
  2. INSERT parent genomic SEQ (+ FEAT_LOCATION pointing seq_no at it, root=contig)
  3. INSERT FEAT_PROPERTY feature_qualifier='Uncharacterized'
  4. INSERT one noncoding_exon child FEATURE (+ SEQ + FEAT_LOCATION + FEAT_RELATIONSHIP
     'part of' rank=2) mirroring how existing sister-species rRNAs are stored.
  Idempotent: a planned feature whose feature_name already exists is skipped.

INPUT:
  --plan : JSON list of rRNA specs produced by build_rrna_plan.py, each:
           {feature_name, gene_name, rrna_class, contig, start, stop, strand,
            headline}  (start/stop already in CGD 1-based coords on `contig`;
            strand 'W'/'C'; for 'C' start>stop is enforced at load time)

Usage:
    python load_rrnas.py --plan cdub_rrna_plan.json [--dry-run]
"""
...
class Inserter:
    ...
    def _insert_feature(self, name, ftype, gene_name, headline) -> int:
        dbxref = self._cal()
        ...
        """), {"o": self.org, "n": name, "x": dbxref, "ft": ftype,
               "g": gene_name, "h": (headline or None) and headline[:240],
               "u": ADMIN_USER})
    ...
    def load_one(self, spec: Dict) -> None:
        ...
        # store minus-strand as start>stop (CGD convention)
        if strand == "C":
        ...
        if self.dry:
            logger.info("[DRY RUN] rRNA %-12s %-9s %s:%d..%d %s len=%d gene=%s",
                        name, spec["rrna_class"], spec["contig"], start, stop,
                        strand, len(res), spec.get("gene_name"))
            return

        parent_no = self._insert_feature(name, "rRNA", spec.get("gene_name"),
                                         spec.get("headline"))
        self._insert_seq_and_location(parent_no, root_seq_no, res, start, stop, strand)
        self._ensure_property(parent_no)

        # single noncoding_exon child (rRNAs are single-exon here)
        ...
        logger.info("inserted rRNA %-12s (feat %d) %s:%d..%d %s len=%d",
                    name, parent_no, spec["contig"], start, stop, strand, len(res))


# --------------------------------------------------------------------------- #
def main() -> None:
    ap = argparse.ArgumentParser(description="Insert rRNA features from a plan")
    ...
    logger.info("=" * 64)
    logger.info("Insert %d rRNA features for %s%s", len(plan), organism_name,
                " [DRY RUN]" if args.dry_run else "")
    logger.info("=" * 64)

    with SessionLocal() as session:
        for tab, col, val, desc in [
            ("FEATURE", "FEATURE_TYPE", "rRNA", "Ribosomal RNA"),
