#!/usr/bin/env python3
"""
Load ORF version aliases into the database.

This script loads ORF name aliases from mapping files (e.g., orf4 to orf19,
orf19 to orf6) into the ALIAS, FEAT_ALIAS, and REF_LINK tables.

Input file format (tab-delimited):
- For orf4: alias_name, orf19_name
- For orf6: orf19_name, contig19, orf6_name, contig6

The alias_type is "Non-uniform" for all loaded aliases.

Original Perl: loadOrf4Aliases.pl, loadOrf6Aliases.pl
...
def get_or_create_alias(
    ...
    logger.debug(f"Created alias: {alias_name}")
...
def parse_orf4_mapping(filepath: Path) -> list[dict]:
    """
    Parse orf4 to orf19 mapping file.

    Format: orf4_name<TAB>orf19_name

    Args:
        filepath: Path to mapping file

    Returns:
        List of dictionaries with alias and feature_name
    """
    entries = []
    ...
    with open(filepath) as f:
        for line in f:
            ...
            if alias_name and feature_name:
                entries.append({
                    "alias": alias_name,
...
def parse_orf6_mapping(filepath: Path) -> list[dict]:
    """
    Parse orf19 to orf6 mapping file.

    Format: orf19_name<TAB>contig19<TAB>orf6_name<TAB>contig6
    ...
    with open(filepath) as f:
        for line in f:
            ...
            feature_name = parts[0].strip()  # orf19
            alias_name = parts[2].strip()     # orf6
...
def load_orf_aliases(
    ...
    """
    Load ORF aliases into the database.
    ...
    for entry in entries:
        ...
        if not feature:
            logger.warning(f"Cannot find feature for: {feature_name}")
        ...
        alias_no = get_or_create_alias(
            session, alias_name, "Non-uniform", created_by
        )
        ...
        if feat_alias_no:
            ...
            logger.info(f"Linked {alias_name} to {feature.feature_name}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Load ORF version aliases from mapping files"
    )
    parser.add_argument(
        "input_file",
        type=Path,
        help="Input mapping file (tab-delimited)",
    )
    parser.add_argument(
        "reference_no",
        type=int,
    ...
    parser.add_argument(
        ...
        choices=["orf4", "orf6"],
        default="orf4",
        help="Input file format (default: orf4)",
    )
    parser.add_argument(
        "--created-by",
    ...
    if args.format == "orf4":
        entries = parse_orf4_mapping(args.input_file)
    else:
        entries = parse_orf6_mapping(args.input_file)
    ...
    try:
        ...
        logger.error(f"Error loading ORF aliases: {e}")
